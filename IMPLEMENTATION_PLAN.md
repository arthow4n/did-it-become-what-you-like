...
- **Reconciled branch/upstream:** `master` is at `ae41417` and is one commit
...
- **Integrated implementation state:** production source fixes through
  ...
  and `ae41417` are present on `master`; the deferred-only SPEC note is
...
- **Current reconciliation evidence:** L-203 and L-201 integration validation
  ...
  duplicate IDs; `master` and its upstream were last aligned at `ae41417`; the
